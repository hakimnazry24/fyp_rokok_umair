...
export default function isNotCounterfeitPage() {
  ...
  const { id, name, manufactured_date, factory } = useLocalSearchParams()
    ...
    <View className="flex h-screen flex-col justify-center ">
      <View>
        ...
        <View className='m-10'>
          ...
          <Text><Text className='font-semibold text-center'>Manufactured Date:</Text> {manufactured_date.toString().split("T")[0]}</Text>
          <Text><Text className='font-semibold text-center'>Factory:</Text> {factory}</Text>
        </View>
